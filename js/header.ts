...
window.addEventListener("DOMContentLoaded", function () {
  ...
  // 멤버메뉴 a 태그들 : 여러 개
  const memberMenuA:NodeListOf<Element> = this.document.querySelectorAll(".member_menu li a");
  //모바일 메뉴 참조
  const mobileHeader:Element | null = this.document.querySelector(".mobile_header");
  // 웹브라우저의 스크롤을 체크하겠다.
